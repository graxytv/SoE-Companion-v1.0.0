...
const SOUND_REF_RE = /\bsound(?:_|\s+)?(\d+)\b/gi;

/**
 * Scan the document for `soundN` / `sound N` references that point to slots which
 * are not configured or are in the `Empty` state. Emits info-severity
...
function scanSoundSlotRefs(doc: import('@codemirror/state').Text): Diagnostic[] {
  ...
  for (let lineNum = 1; lineNum <= doc.lines; lineNum++) {
    ...
    while ((m = SOUND_REF_RE.exec(text)) !== null) {
      ...
      if (!Number.isFinite(n) || n < 1 || !Number.isSafeInteger(n)) continue;
