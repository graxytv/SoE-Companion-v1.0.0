...
pub struct ItemDropEvent {
    ...
    /// Stable data-table/base item name before loot-filter display rewriting.
    /// For unique/set drops this is intentionally left empty because SoE/PD2
    /// unique file indexes are not reliable enough for exact identity.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    ...
    /// the main loop fires `loot-history-entry` to the frontend and the
    /// overlay increments tracker totals. Dedup-merges should not count twice.
    #[serde(default)]
...
pub struct DropScanner {
    ...
    /// Inventory unique/set items already announced after becoming identified.
    identified_inventory_announced: HashSet<u32>,
    /// Unidentified unique/set items seen by this session and eligible for an
    /// identify-time announcement once they become identified in inventory.
    inventory_identify_candidates: HashSet<u32>,
    /// Fallback identify candidates keyed by `(item class, quality)`. This is
    /// used when Diablo changes the unit/seed key across Save & Exit.
    inventory_identify_candidate_fingerprints: HashMap<(u32, u32), u32>,
    ...
    /// Temporary quiet period after entering a game. PD2 can expose inventory,
    /// equipped, charm, or stale transition items through room chains during
    /// character load. We still warm seen-item state, but do not emit item
    /// events, push loot history, or create identify-time candidates.
    suppress_item_events_until: Option<Instant>,
}
...
impl DropScanner {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            identified_inventory_announced: HashSet::new(),
            inventory_identify_candidates: HashSet::new(),
            inventory_identify_candidate_fingerprints: HashMap::new(),
            ...
            suppress_item_events_until: None,
        })
    }
    ...
    pub fn clear_cache(&mut self) {
        ...
        self.identified_inventory_announced.clear();
        self.missed_ticks.clear();
        self.seen_goblins.clear();
        self.last_pickup_updates.clear();
        self.last_goblin_events.clear();
        self.suppress_item_events_until = None;
    ...
    pub fn suppress_item_events_for(&mut self, duration: Duration) {
        self.suppress_item_events_until = Some(Instant::now() + duration);
    }

    fn item_events_suppressed(&mut self) -> bool {
        if let Some(until) = self.suppress_item_events_until {
    ...
    fn apply_filter_decision(&mut self, event: &mut ItemDropEvent, apply_visibility: bool) -> bool {
        ...
        let Some(filter_arc) = filter_arc else {
            return false;
        };
        let Ok(filter) = filter_arc.read() else {
            return false;
        };
        ...
        if apply_visibility && self.loot_hook.is_injected() {
            match decision.visibility {
                ...
                Visibility::Hide => {
                    ...
                    {
                        log_error(&format!("Failed to hide item {}: {}", event.unit_id, e));
                    }
                }
                Visibility::Default => {}
            }
    ...
    fn item_event_key(event: &ItemDropEvent) -> u32 {
        if event.seed != 0 {
            event.seed
        } else {
            event.unit_id
    ...
    fn identify_fingerprint(class: u32, quality: u32) -> (u32, u32) {
        (class, quality)
    }

    fn add_inventory_identify_candidate(&mut self, event: &ItemDropEvent) {
        self.inventory_identify_candidates
            .insert(Self::item_event_key(event));
        let fingerprint = Self::identify_fingerprint(
            event.class,
            if event.quality.eq_ignore_ascii_case("set") {
                item_quality::SET
            } else {
                item_quality::UNIQUE
            },
        ...
        *self
            .inventory_identify_candidate_fingerprints
            .entry(fingerprint)
            .or_insert(0) += 1;
    }
    ...
    fn consume_inventory_identify_candidate(&mut self, key: u32, class: u32, quality: u32) -> bool {
        ...
        let fingerprint = Self::identify_fingerprint(class, quality);
        let Some(count) = self
            .inventory_identify_candidate_fingerprints
            .get_mut(&fingerprint)
        else {
        ...
        *count -= 1;
        if *count == 0 {
            self.inventory_identify_candidate_fingerprints
                .remove(&fingerprint);
        }
        true
    }

    fn is_unidentified_unique_or_set(event: &ItemDropEvent) -> bool {
        !event.is_identified
            && (event.quality.eq_ignore_ascii_case("unique")
                || event.quality.eq_ignore_ascii_case("set"))
    }
    ...
    fn is_tracker_relevant_event(event: &ItemDropEvent) -> bool {
        ...
        Self::is_fate_card_event(event)
            ...
            || Self::contains_tracker_keyword(event, "token")
    ...
    fn contains_tracker_keyword(event: &ItemDropEvent, needle: &str) -> bool {
        event.name.to_ascii_lowercase().contains(needle)
            || event.base_name.to_ascii_lowercase().contains(needle)
            || event.canonical_name.to_ascii_lowercase().contains(needle)
            || event
                .category
                .as_deref()
                .unwrap_or_default()
    ...
    pub fn tick_items(&mut self) -> Vec<ItemDropEvent> {
        ...
        let inventory_item_ids = self.read_player_inventory_ids();
        let suppress_item_events = self.item_events_suppressed();
        ...
        for i in 0..i_paths {
            ...
            while p_unit != 0 {
                ...
                if unit.unit_type == unit_type::ITEM {
                    // The room unit list can include items linked to the local
                    // player's inventory during game-load/area sync. Those are
                    ...
                    if should_emit && !suppress_item_events {
                        ...
                        if event.filter.is_some() || tracker_relevant {
                            ...
                            let entry = crate::loot_history::LootEntry {
                                ...
                                seed: event.seed,
                                unique_kind: event.unique_kind.as_ref().map(|k| {
                                    match k {
                                        UniqueKind::Tu => "tu",
                                        UniqueKind::Su => "su",
                                        UniqueKind::Ssu => "ssu",
                                ...
                                is_relic: event.name.contains("Relic")
                                    || event.base_name.contains("Relic"),
                            };
                            ...
                            if event.history_pushed && unidentified_unique_or_set {
                                self.add_inventory_identify_candidate(&event);
                            }
                        }
                        events.push(event);
                    }
        ...
        if !suppress_item_events {
            events.extend(self.scan_identified_inventory_events());
        }

        events
    }

    fn scan_identified_inventory_events(&mut self) -> Vec<ItemDropEvent> {
        let mut events = Vec::new();
        for (p_item, unit) in self.read_player_inventory_units() {
            if unit.p_unit_data == 0 {
                continue;
            }
            let Ok(item_data) = self
                .state
                .ctx
                .process
            ...
            let key = self
                ...
                .ctx
                .process
                .read_memory::<u32>(unit.p_unit_data as usize + item_data::SEED)
                .unwrap_or(unit.unit_id);
            let key = if key == 0 { unit.unit_id } else { key };
            if !self.consume_inventory_identify_candidate(key, unit.class, item_data.quality) {
                continue;
            }
            if !self.identified_inventory_announced.insert(key) {
                continue;
            }

            let mut scanned = ScannedItem::from_unit(&unit, &item_data, p_item);
            {
            ...
            event.source = "identify-inventory".to_string();
            event.history_pushed = true;
            let _ = self.apply_filter_decision(&mut event, false);
            events.push(event);
        }
        events
    }
    ...
    fn scan_unit(&mut self, p_unit: u32, unit: &UnitAny) -> Option<ScannedItem> {
        ...
        // Only actual ground labels are drops. Inventory/equipment/socketed
        // item units can be visible in room chains during game creation and
        // must not count as newly dropped items.
        if unit.mode != ITEM_MODE_ON_GROUND {
            return None;
        }
        ...
        if (scanned.quality == item_quality::UNIQUE || scanned.quality == item_quality::SET)
            && scanned.is_identified
            && scanned
                .name
                .as_deref()
                .map(str::trim)
    ...
    fn to_event(&self, scanned: ScannedItem) -> ItemDropEvent {
        ...
        let table_name = match scanned.quality {
        ...
        let table_name = table_name
            ...
            .filter(|name| !name.is_empty());
        let base_fallback = if base_name.is_empty() {
            None
        } else {
        ...
        let (name, name_source) = if let Some(name) = fate_card_name.clone() {
            ...
            (name, "data-table")
        } else if let Some(name) = base_fallback {
            (name, "base-name")
        } else {
            (format!("Item #{}", scanned.class), "item-class")
        };
        ...
        let is_hellforged = scanned.quality == item_quality::UNIQUE
            && (unique_kind == Some(UniqueKind::Ssu)
                || unique_kind == Some(UniqueKind::Sssu)
                || name.to_ascii_lowercase().contains("hellforged"));
        // Stable identity used by companion tracking. Unique/set exact names
        // must come from the live tooltip because PD2/SoE table indexes are
        ...
        ItemDropEvent {
            ...
            is_runeword: scanned.is_runeword,
            p_unit_data: scanned.p_unit_data,
            mode: scanned.mode,
            file_index: scanned.file_index,
            seed,
            history_pushed: false,
            source: "ground-scan".to_string(),
    ...
    fn class_code(&self, class: u32) -> String {
        self.class_cache
            .as_ref()
            .and_then(|cache| cache.get(class as usize))
    ...
    fn build_class_cache(&self) -> Result<Vec<ClassInfo>, String> {
        ...
        for class in 0..count {
            ...
            let code = self
                ...
                .ctx
                .process
                .read_memory::<u32>(record + items_txt::CODE)
                .map(item_code_from_u32)
                .unwrap_or_default();
    ...
    fn read_player_inventory_ids(&self) -> HashSet<u32> {
        self.read_player_inventory_units()
            .into_iter()
            .map(|(_, unit)| unit.unit_id)
            .collect()
    }

    fn read_player_inventory_units(&self) -> Vec<(u32, UnitAny)> {
        ...
        for _ in 0..256 {
            ...
            if let Ok(unit) = self
                ...
                .ctx
                .process
                .read_memory::<UnitAny>(p_item as usize)
            {
                units.push((p_item, unit));
            }
...
#[cfg(target_os = "windows")]
fn item_code_from_u32(value: u32) -> String {
    value
        .to_le_bytes()
        .iter()
        .copied()
