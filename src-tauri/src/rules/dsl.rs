...
fn parse_attrs_into(
    ...
    while i < tokens.len() {
        ...
        let next_lower = tokens.get(i + 1).map(|t| t.to_lowercase());

        if let Some(consumed) = apply_compound_notify_token(&lower, next_lower.as_deref(), attrs) {
            i += consumed;
            continue;
        }
        ...
        if lower == "sound" {
            if let Some(num) = next_lower.as_deref().and_then(parse_sound_number) {
...
fn apply_compound_notify_token(
    lower: &str,
    next_lower: Option<&str>,
    attrs: &mut Attrs,
) -> Option<usize> {
    match lower {
        "notify_map" | "map_notify" => {
            attrs.notify = Some(true);
            attrs.map = Some(true);
            Some(1)
        }
        "notify_sound" => {
            attrs.notify = Some(true);
            if let Some(num) = next_lower.and_then(parse_sound_number) {
                attrs.sound = Some(num);
                Some(2)
            } else {
                Some(1)
        ...
        "notify_map_sound" | "notify_sound_map" | "map_notify_sound" | "map_sound_notify" => {
            attrs.notify = Some(true);
            attrs.map = Some(true);
            if let Some(num) = next_lower.and_then(parse_sound_number) {
                attrs.sound = Some(num);
        ...
        _ => {
            if let Some(rest) = lower.strip_prefix("notify_sound") {
                attrs.notify = Some(true);
                if let Some(num) = parse_sound_number(rest.trim_start_matches('_')) {
                    attrs.sound = Some(num);
                }
                Some(1)
            } else if let Some(rest) = lower.strip_prefix("map_sound") {
                attrs.map = Some(true);
                if let Some(num) = parse_sound_number(rest.trim_start_matches('_')) {
                    attrs.sound = Some(num);
                }
                Some(1)
            } else if let Some(rest) = lower.strip_prefix("notify_map_sound") {
                attrs.notify = Some(true);
                attrs.map = Some(true);
                if let Some(num) = parse_sound_number(rest.trim_start_matches('_')) {
                    attrs.sound = Some(num);
...
fn parse_sound_keyword(lower: &str) -> Option<u32> {
    let suffix = lower.strip_prefix("sound")?.trim_start_matches('_');
    parse_sound_number(suffix)
}

fn parse_sound_number(s: &str) -> Option<u32> {
...
fn validate_tokens(
    ...
    while i < tokens.len() {
        ...
        let lower = token.to_lowercase();
        let next_lower = tokens.get(i + 1).map(|t| t.to_lowercase());
        if let Some(consumed) = known_compound_token(&lower, next_lower.as_deref()) {
            i += consumed;
            continue;
        }
...
fn is_known_token(lower: &str) -> bool {
    ...
    matches!(
        lower,
        "eth" | "show" | "hide" | "notify" | "stat" | "sound_none" | "map" | "sound"
    ) || parse_sound_keyword(lower).is_some()
}

fn known_compound_token(lower: &str, next_lower: Option<&str>) -> Option<usize> {
    if lower == "sound" {
        return next_lower.and_then(parse_sound_number).map(|_| 2);
    }
    match lower {
        "notify_map" | "map_notify" => Some(1),
        "notify_sound" | "map_sound" | "notify_map_sound" | "notify_sound_map"
        | "map_notify_sound" | "map_sound_notify" => {
            Some(if next_lower.and_then(parse_sound_number).is_some() {
                2
            } else {
                1
            })
        }
        _ if lower.starts_with("notify_sound")
            || lower.starts_with("map_sound")
            || lower.starts_with("notify_map_sound") =>
        {
            Some(1)
        }
        _ => None,
...
fn scan_notify_flags(src: &str) -> NotifyFlags {
    ...
    while i < tokens.len() {
        ...
        if let Some(consumed) =
            scan_compound_notify_flags(&lower, next_lower.as_deref(), &mut flags)
        {
            i += consumed;
            continue;
        }
        if NotifyColor::from_str(&lower).is_some() {
            ...
            flags.sound = true;
        } else if lower == "sound" && next_lower.as_deref().and_then(parse_sound_number).is_some() {
            flags.sound = true;
            i += 2;
            continue;
...
fn scan_compound_notify_flags(
    lower: &str,
    next_lower: Option<&str>,
    flags: &mut NotifyFlags,
) -> Option<usize> {
    match lower {
        "notify_map" | "map_notify" => {
            flags.notify = true;
        ...
        "notify_sound" => {
            flags.notify = true;
            flags.sound = flags.sound || next_lower.and_then(parse_sound_number).is_some();
            Some(if next_lower.and_then(parse_sound_number).is_some() {
                2
            } else {
                1
        ...
        _ if lower.starts_with("notify_sound") => {
            ...
            flags.sound = flags.sound
                || parse_sound_number(
                    lower
                        .trim_start_matches("notify_sound")
                        .trim_start_matches('_'),
                )
                .is_some();
            Some(1)
        }
        _ if lower.starts_with("map_sound") => {
            flags.sound = flags.sound
                || parse_sound_number(
                    lower
                        .trim_start_matches("map_sound")
                        .trim_start_matches('_'),
                )
                .is_some();
            Some(1)
        }
        _ if lower.starts_with("notify_map_sound") => {
            flags.notify = true;
            flags.sound = flags.sound
                || parse_sound_number(
                    lower
                        .trim_start_matches("notify_map_sound")
                        .trim_start_matches('_'),
                )
                .is_some();
            Some(1)
...
"foo" sound1"#;
  ...
  "Tyrael"
    ...
    #[test]
    fn parse_sound_accepts_spaced_and_underscore_forms() {
        let cfg = parse_dsl("su notify sound 12").unwrap();
        assert_eq!(cfg.rules[0].tiers, vec![ItemTier::Su]);
        assert!(cfg.rules[0].notify);
        assert_eq!(cfg.rules[0].sound, Some(12));

        let cfg = parse_dsl("ssu notify map sound_9").unwrap();
        assert_eq!(cfg.rules[0].tiers, vec![ItemTier::Ssu]);
        assert!(cfg.rules[0].notify);
        assert!(cfg.rules[0].map);
        assert_eq!(cfg.rules[0].sound, Some(9));

        let cfg = parse_dsl("sssu notify_map_sound 16").unwrap();
        assert_eq!(cfg.rules[0].tiers, vec![ItemTier::Sssu]);
        assert!(cfg.rules[0].notify);
        assert!(cfg.rules[0].map);
        assert_eq!(cfg.rules[0].sound, Some(16));
    }

    #[test]
    fn parse_sound_rejects_zero_but_accepts_high_custom_slots() {
        // sound0 is not a valid keyword.
        let cfg = parse_dsl("\"X\" notify sound0").unwrap();
        assert_eq!(cfg.rules[0].sound, None);

        // Custom sound slots are user-expandable in the Sounds tab, so the
        // DSL must not cap them at the original built-in/default range.
        let cfg = parse_dsl("\"X\" notify sound256").unwrap();
        assert_eq!(cfg.rules[0].sound, Some(256));
        let cfg = parse_dsl("\"X\" notify sound1024").unwrap();
        assert_eq!(cfg.rules[0].sound, Some(1024));
    }
}
