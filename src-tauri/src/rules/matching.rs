...
use crate::notifier::{ItemDropEvent, UniqueKind};
...
impl<'a> MatchContext<'a> {
    ...
    fn tiers_match(&self, rule_tiers: &[ItemTier]) -> bool {
        ...
        rule_tiers.iter().any(|&rule_tier| match rule_tier {
            // SU/SSU/SSSU are unique-tier classifiers rather than base-item tiers.
            // This lets filter rules like `su notify sound1`, `ssu notify sound2`,
            // and `sssu notify sound3` target those Median XL unique buckets directly.
            ItemTier::Su => self.item.unique_kind == Some(UniqueKind::Su),
            ItemTier::Ssu => self.item.unique_kind == Some(UniqueKind::Ssu),
            ItemTier::Sssu => self.item.unique_kind == Some(UniqueKind::Sssu),
            _ => self.item.tier == Some(rule_tier),
        })
    }
