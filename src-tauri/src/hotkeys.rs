...
impl LootHistoryHotkeyState {
    ...
    pub fn start(&self, app_handle: AppHandle, hotkey: HotkeyConfig) {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            log_info("Loot-history watcher is only supported on Windows");
            let _ = (app_handle, current_hotkey);
        }
    }
...
/// Watcher for the "reset Drops Tracker" hotkey. Polls the configured key
/// every ~30ms and emits `reset-drops-tracker` to the overlay/main webviews on
/// rising-edge press. The frontend clears the shared loot history state.
pub struct ResetDropsTrackerHotkeyState {
    is_running: Arc<AtomicBool>,
    current_hotkey: Arc<std::sync::Mutex<HotkeyConfig>>,
}

impl ResetDropsTrackerHotkeyState {
    pub fn new() -> Self {
        Self {
            is_running: Arc::new(AtomicBool::new(false)),
            current_hotkey: Arc::new(std::sync::Mutex::new(HotkeyConfig {
    ...
    pub fn start(&self, app_handle: AppHandle, hotkey: HotkeyConfig) {
        if self.is_running.load(Ordering::SeqCst) {
            log_info("Reset-Drops-Tracker watcher already running, restarting with new config");
            self.stop();
            thread::sleep(std::time::Duration::from_millis(80));
        }
        ...
        {
            thread::spawn(move || {
                reset_drops_tracker_hotkey_thread_windows(is_running, current_hotkey, app_handle);
            });
        }

        #[cfg(not(target_os = "windows"))]
        {
            log_info("Reset-Drops-Tracker watcher is only supported on Windows");
            let _ = (app_handle, current_hotkey);
        }
    }
...
impl Default for ResetDropsTrackerHotkeyState {
    fn default() -> Self {
        Self::new()
    }
}
...
#[cfg(target_os = "windows")]
fn reset_drops_tracker_hotkey_thread_windows(
    is_running: Arc<AtomicBool>,
    current_hotkey: Arc<std::sync::Mutex<HotkeyConfig>>,
    app_handle: AppHandle,
) {
    log_info("Reset-Drops-Tracker hotkey watcher thread starting");
    ...
    while is_running.load(Ordering::SeqCst) {
        ...
        if active && !prev_down {
            if let Err(e) = app_handle.emit("reset-drops-tracker", ()) {
                log_error(&format!("Failed to emit reset-drops-tracker: {}", e));
            }
        }
        prev_down = active;
    ...
    log_info("Reset-Drops-Tracker hotkey watcher thread stopped");
}

#[tauri::command]
pub fn update_reset_drops_tracker_hotkey(
    state: tauri::State<ResetDropsTrackerHotkeyState>,
    app: AppHandle,
    hotkey: HotkeyConfig,
) -> Result<(), String> {
    log_info(&format!(
        "Updating reset-drops-tracker hotkey to: {}",
        hotkey.display
    ));
    state.start(app, hotkey);
    Ok(())
...
/// Watcher for the "toggle Muling Mode" hotkey. Polls the configured key
/// every ~30ms and emits `toggle-muling-mode` on rising-edge press.
pub struct MulingModeHotkeyState {
    is_running: Arc<AtomicBool>,
    current_hotkey: Arc<std::sync::Mutex<HotkeyConfig>>,
}

impl MulingModeHotkeyState {
    pub fn new() -> Self {
        Self {
            is_running: Arc::new(AtomicBool::new(false)),
            current_hotkey: Arc::new(std::sync::Mutex::new(HotkeyConfig {
    ...
    pub fn start(&self, app_handle: AppHandle, hotkey: HotkeyConfig) {
        if self.is_running.load(Ordering::SeqCst) {
            log_info("Muling-mode watcher already running, restarting with new config");
            self.stop();
            thread::sleep(std::time::Duration::from_millis(80));
        }
        ...
        {
            thread::spawn(move || {
                muling_mode_hotkey_thread_windows(is_running, current_hotkey, app_handle);
            });
        }

        #[cfg(not(target_os = "windows"))]
        {
            log_info("Muling-mode watcher is only supported on Windows");
            let _ = (app_handle, current_hotkey);
        }
    }
...
impl Default for MulingModeHotkeyState {
    fn default() -> Self {
        Self::new()
    }
}
...
#[cfg(target_os = "windows")]
fn muling_mode_hotkey_thread_windows(
    is_running: Arc<AtomicBool>,
    current_hotkey: Arc<std::sync::Mutex<HotkeyConfig>>,
    app_handle: AppHandle,
) {
    log_info("Muling-mode hotkey watcher thread starting");
    ...
    while is_running.load(Ordering::SeqCst) {
        ...
        if active && !prev_down {
            if let Err(e) = app_handle.emit("toggle-muling-mode", ()) {
                log_error(&format!("Failed to emit toggle-muling-mode: {}", e));
            }
        }
        prev_down = active;
    ...
    log_info("Muling-mode hotkey watcher thread stopped");
}

#[tauri::command]
pub fn update_muling_mode_hotkey(
    state: tauri::State<MulingModeHotkeyState>,
    app: AppHandle,
    hotkey: HotkeyConfig,
) -> Result<(), String> {
    log_info(&format!(
